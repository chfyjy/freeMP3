...
ID3V2_3Getter::ID3V2_3Getter(const QString& filepath)
{
    ...
    char* framedata = new char[ID3V2_TagFrameL];
    int pos = getTagFrame(idMap.key(35).toLocal8Bit(), framedata);
    ID3V2_TagFrame tmpframe = initTagFrame(framedata);
    DEBUGOUT(tmpframe.id);
    DEBUGOUT(tmpframe.isize);
}
...
void ID3V2_3Getter::initID3V2_header(const QString& filepath)
{
    ...
    if(strncmp(header2.header, "ID3", 3) == 0)
        id3Valid = true;
    else
        return;
...
void ID3V2_3Getter::initID3V1_header(int tagid, const char* tagdata, int len)
{
    switch (tagid)
    {
    case 35:memcmp(header1.title, tagdata, len);
        DEBUGOUT(header1.title);
        break;
    }
}

int ID3V2_3Getter::getTagFrame(const char* tag, char* tagdata)
{
    DEBUGOUT(tag);
    int pos = allframe.indexOf(QString(tag).toUtf8());
    DEBUGOUT(pos);
    DEBUGOUT(allframe.left(10));
    char* data = allframe.toUtf8().data();
    memcpy(tagdata, data+pos, ID3V2_TagFrameL);
    return pos;
}
...
void ID3V2_3Getter::readTagFrame(const QString& filepath)
{
    ...
    int readlen = ID3V2_TagFrame_size - ID3V2_headerL;
    reader.seek(ID3V2_headerL);
    allframe = reader.read(readlen);
    QString frametmp, frame, framedata;
    frametmp = allframe;
    DEBUGOUT(allframe);
    file.close();
}
